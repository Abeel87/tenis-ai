...
**TRAINING ARCHIVE STAGED-MANIFEST REPAIR (2026-09-24):** latest GC run `35970872835` is GREEN after `training_archive_gc_prepare_timeout`; preview retained the newest 2 complete manifests, found 0 active pins, 1035 candidates / `147406562` B and 52 grace-ready objects / `16032341` B. Apply audit #7 completed safely with 0 planned/deleted objects and 0 deleted bytes. Production DB and Storage agree on 12332 objects / `249736443` B, but inventory attempt 2 (`35958393373`) still fails because its required reservation is `250790282 > 250000000` B. Root cause is 39 failed publications left permanently `staged`; their partial entries pin old objects, so apply cannot legally plan the otherwise grace-ready candidates. Branch `fix/training-archive-abandoned-manifests` adds a private, transactional `abandoned` lifecycle: the authenticated publisher releases only its own failed partial manifest, and GC reaps staged manifests only after 24h. Neither path deletes physical objects; the normal latest-2, pins and fresh 168h eligibility clock remain mandatory. Focused tests are 28/28 GREEN. NEXT: publish PR, require full green CI, merge, apply the migration and deploy both canonical Edge Functions; then rerun GC and verify audit. Do not raise the cap, shorten grace, remove pins or directly delete objects.
...
**ACTIVE BRANCH:** `fix/training-archive-abandoned-manifests` for the bounded failed-publication lifecycle repair. No model, probability, threshold or iNeed$ calculation changes.
...
1. PR #490 and production observer timeout migration are completed and production-verified. Let the next scheduled GC run at 14:20 UTC verify the matured 168h gate, then reconcile latest-two complete manifests, pins, staged/in-flight references, deleted counts and bytes against apply audit; rerun Training Archive if the gate passes.\n2. Do not redo PR #476, the Supabase migration, Edge Function deployment, or the pre-grace manual GC; all are already proven complete.
3. Until `2026-09-23T14:05:13.238755Z`, Training Archive GC must remain a factual no-delete state. Any deletion before that timestamp is a blocker.
4. After the 168h grace matures, let/dispatch `Training archive retention GC`, verify latest-2 complete manifests, active pins and all staged/in-flight references remain intact, reconcile reclaimed bytes and object counts against `training_archive_gc_apply_audit`, then rerun Training Archive. Do not raise the 250 MB archive cap or shorten grace as a workaround.
