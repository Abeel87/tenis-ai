...
**INEED$ MATCH-WINNER SCOPE (2026-09-24, in progress):** User confirmed V1 should evaluate who wins the match rather than first-set/game totals. Audit found `backend/ineed_scope.py` filtered out `match_winner` despite 17 final PLAYABLE winner signals in the 07:32 UTC source snapshot; last production sync at 07:34 UTC evaluated 19 first-set overs only. The EV formula itself reconciles (e.g. 85.96% × 1.05 × 0.88 − 1 = −20.57%), but the old DB SHADOW insert trigger also rejects match winner. Isolated branch changes V1 intake and that trigger to match-winner-only, keeps old bet settlement and the independent builder SHADOW input scope, leaves model/EV/tax/risk/stake/thresholds untouched. Fresh exact Direct quote still gates every candidate. Focused tests 34/34 GREEN. NEXT: full regression, PR/full green CI, fresh-main check, merge, deploy trigger migration, then production SHADOW run and audit actual winner outcomes/offer coverage. Do not assert the old 07:32 positive-EV quote is still live.
