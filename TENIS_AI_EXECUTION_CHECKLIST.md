...
**DYNAMIC QUOTE PREFLIGHT:** live main `35988bad...` contains 376 result rows, 33 final Symphony playable compositions (all 2-leg), 3 verified Direct matches and 2 composition/Direct overlaps. Only one overlap has every required component UUID; 0/33 final compositions have an exact Phase-2 pre-priced combination quote. PR #466 therefore adds a bounded exact dynamic-SGA sidecar producer and does not wire that producer into scheduled refresh/runtime before the `15063427` time gate. During PR CI, legacy `Superbet Direct Source Probe` run `35713368211` nevertheless executed its existing live browser/direct steps and read public event `15063427` at approximately `2026-09-22T09:59Z` (11:59 CEST), before the intended 14:00 CEST gate. This was not a dynamic-SGA or terminal-result request and had zero production influence, but it violated the session's no-live-request gate. PR #466 therefore guards both live Direct workflow steps with `github.event_name != 'pull_request'` and adds a regression test; a follow-up audit also found PR-triggered `Superbet hourly market refresh` would return `refresh=true`. Run `35713798455` was cancelled before its live refresh steps, and the canonical `superbet_refresh_guard.py` now returns `refresh=false` with reason `pull_request_validation_only`, causing every refresh-dependent step to skip on PR while the orchestration guard test remains active.
