...
def main():
    ...
    req(css,'.mt84e2','brak stylów E2 w style.css'); req(idx,'href="style.css"','index nie ładuje kanonicznego style.css'); req(idx,'model-trends.js','index nie ładuje kanonicznego JS Trend Monitor')
    if 'model-trends.css' in idx or (ROOT/'frontend/model-trends.css').exists(): ERR.append('stary osobny CSS Trend Monitor nie może wrócić')
    req(idx,'href="style.css"','brak kanonicznego CSS'); req(idx,'autolearn-v84.js?v=84a1&hf=84b1','naruszono chroniony AutoLearn JS pin')
    if 'autolearn-v84.css' in idx or (ROOT/'frontend/autolearn-v84.css').exists(): ERR.append('stary osobny AutoLearn CSS nie może wrócić')
