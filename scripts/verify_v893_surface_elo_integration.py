...
def main():
    ...
    print(json.dumps({'status': 'PASS', 'version': 'v8.9.3', 'report_status': report.get('status'), 'events': report.get('elo', {}).get('events'), 'coverage': report.get('elo', {}).get('rows_with_both_surface_history'), 'training_rows': report.get('training', {}).get('rows_total'), 'holdout_rows': report.get('training', {}).get('holdout_rows'), 'gates': {k: v.get('status') for k, v in report.get('gates', {}).items()}, 'production_influence': False}, ensure_ascii=False))
    return 0
