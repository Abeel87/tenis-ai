...
def test_fixture_history_runtime_loaded_after_data_runtime_before_symphony2():
    ...
    assert h.index("fixture-history-freshness.js") < h.index('src="symphony2.js"')
    assert "runtime-health-v84e0.js" not in h
    assert "hotfix-v84e01.js" not in h


def test_history_refresh_is_lightweight():
    s=read("frontend/fixture-history-freshness.js")
...
def test_status_filter_contract_with_node_when_available():
    ...
    if not node:
        return
    script=r'''
const h=require("./frontend/fixture-history-freshness.js");
const bad=[
...
def test_fixture_history_runtime_does_not_modify_model_math_files():
    s=read("frontend/fixture-history-freshness.js")
