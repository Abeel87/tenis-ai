...
def test_clean_product_assets_and_brand_are_wired():
    h=t("frontend/index.html")
    assert "<title>Tenis AI</title>" in h
    assert 'href="style.css"' in h
    assert "ui-cleanup.css" not in h
    assert "ui-cleanup.js" not in h
    assert not (ROOT/"frontend/ui-cleanup.js").exists()
    assert not (ROOT/"frontend/ui-cleanup.css").exists()

def test_stats_ranking_is_wired_after_stats_owners():
    h=t("frontend/index.html")
    js=t("frontend/stats-ranking.js")
    assert "stats-ranking.js" in h
    assert h.index("performance-dashboard.js") < h.index("stats-ranking.js")
...
def test_market_quality_layer_is_scoped_to_core_and_cross_view():
    ...
    for token in [
        "CP_MIN_SETTLED=30","CP_MIN_ACCURACY=65","CP_MIN_WILSON=45",
        "RESULT_MIN_SETTLED=30","RESULT_MIN_ACCURACY=65","RESULT_MIN_WILSON=45",
        "RESULT_MARKETS=new Set([...WINNER_MARKETS,'total_sets'])",
        "early_hold_v7?.ready!==true","segments_30d?.market",
        "window.TENIS_AI_WINNER_QUALITY_V888","window.TENIS_AI_RESULT_QUALITY_V889",
        "window.TENIS_AI_CROSS_VIEW_QUALITY_V8810",
        "return state.coreEventDepth>0?filteredSignals(rows,match):rows",
        "activeScenarioProfile()!=='experimental'","finalSelectedSignals(match,limit=3)",
        "window.bestSignalsData=(match,limit=3)=>finalSelectedSignals",
        "window.bestSignals=match=>","window.compactSignals=match=>",
...
def test_retired_cleanup_and_scenario_decorators_stay_deleted():
    assert not (ROOT/"frontend/ui-cleanup.js").exists()
    assert not (ROOT/"frontend/ui-cleanup.css").exists()
    ui=t("frontend/project-ui.js")
    css=t("frontend/style.css")
    for token in ("PAIR SELECTOR · RANKING","DLACZEGO TEN MECZ","selector_pair","decorateGeneratorCards","wrapScenarioOpen","TENIS_AI_SCENARIOS","scenario-v82a-panel"):
        assert token not in ui
    assert ".sc883-pairbar" not in css
    assert ".sc883-score-note" not in css

def test_single_stats_owner_and_legacy_ui_removed():
...
def test_shadow_boundary_copy_remains():
    ui=t("frontend/project-ui.js")
    assert "window.TENIS_AI_APPLY_META?.()" in ui
