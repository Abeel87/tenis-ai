...
def test_v80_is_central_version():
    ...
    assert 'clean-core.js?v=801' in idx
    assert 'clean-core.css?v=801' in idx
...
def test_history_has_one_v8_runtime_owner():
    idx=read('frontend/index.html'); clean=read('frontend/clean-core.js')
...
def test_active_legacy_bridges_keep_required_features():
    ...
    assert 'data-shadow-open' not in ui
