...
assert.deepEqual(styles,['style.css'],'Only the canonical stylesheet may load');
const historyMarker='/* History: canonical new-shell presentation, scoped and read-only. */';
assert(style.includes(historyMarker),'Canonical stylesheet must retain the History presentation section');
const historyStyle=style.split(historyMarker,2)[1]||'';
assert(historyStyle.includes('[data-rich-history-root]'),'History styles must stay scoped to the History view');
assert(!historyStyle.includes('!important'),'History styles must not force global overrides');
assert(!fs.existsSync(path.join(root,'history-modern.css')),'History must not reintroduce a second stylesheet');
...
assert(app.includes("serviceWorker.register('sw.js'")&&app.includes("updateViaCache:'none'")&&app.includes('r.update()'),'App checks for current PWA worker');
assert(sw.includes("fetch(r,{cache:'no-store'})")&&sw.includes('client.navigate(client.url)'),'Worker refreshes controlled pages with fresh static assets');
...
for(const f of [...scripts,...styles,'manifest.webmanifest','favicon.png','brand-symbol.png'])assert(sw.includes(`'${f}'`),'PWA core missing runtime asset '+f);
