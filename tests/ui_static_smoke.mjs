...
const index=read('index.html'),style=read('style.css'),historyStyle=read('history-modern.css'),app=read('app.js'),ineed=read('ineed.js'),auth=read('account.js'),sw=read('sw.js'),manifest=JSON.parse(read('manifest.webmanifest'));
...
assert.deepEqual(styles,['style.css','history-modern.css'],'Only the canonical base stylesheet and scoped History stylesheet may load');
assert(historyStyle.includes('[data-rich-history-root]'),'History stylesheet must stay scoped to the History view');
assert(!historyStyle.includes('!important'),'History stylesheet must not force global overrides');
