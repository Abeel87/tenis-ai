...
## R12/R13 merge closeout / R18-R19 active

PR #411 final head `24ef32e6649b0b07c8d40ad6c460e76a47cfe8cd` merged as `0f200405f884389404d351a9264a534c7f78b00e` after all 12 exact-head workflows passed. Point Tape `35458629267` completed 78/78 GREEN; Player DNA `35458629264` SUCCESS; UI & Project Health full suite reported 1297/1297 GREEN.

Post-merge Update-and-Pages `35462310243` completed SUCCESS, including deploy Pages, and published bot commit `4911831dd6970486b6025b7cae6b893737c21047`. Independent production proof: 115 result rows, exactly 44 rows with legacy `model_ready=true`, and `meta.model_ready=44`. `meta.semantic_readiness_shadow` correctly emitted `available=false`, `status=N/D`, `reason=RESULTS_SNAPSHOT_MISMATCH` because the preceding sidecar belonged to the earlier 109-match results snapshot. No semantic zero was fabricated.

R18/R19 is the active observability-only step on branch `logic-08-readiness-ui-observability`. The canonical backend projection now owns `dimension_state_totals` derived only from validated `dimension_status_counts`; malformed counts fail closed as `READINESS_DIMENSION_COUNTS_INVALID`. The admin overview displays these backend-owned SHADOW dimension-state totals separately from legacy `Modele gotowe`. Diagnostics display backend per-dimension READY/NOT_READY/UNKNOWN counts. The browser does not build readiness, infer an overall-ready match count, or read the sidecar directly.

R16 moderator problem filtering remains byte-for-byte legacy in behavior (`problem || !m.model_ready`) and has a dedicated contract test. Current post-rebase R18/R19 validation: 42/42 readiness/matrix/consumer tests GREEN; full pytest 1299/1299 GREEN; `py_compile` and JS syntax GREEN; current real N/D UI smoke GREEN; synthetic aligned available=true smoke GREEN; synthetic N/D smoke GREEN; `git diff --check` GREEN.
