...
def enrich(m):
    ...
    m["market_lab_v741"]={
      ...
      "player_total_games":{p1:player_lines(joint,0),p2:player_lines(joint,1)},
...
def main():
    ...
    for m in rows:
        x=enrich(dict(m))
        if x.get("market_lab_v741"):ready+=1
        out.append(x)
    write(RESULTS,out)
    meta=read(META,{})
    if isinstance(meta,dict):
        meta["market_lab_v741_ready"]=ready
        meta["market_lab_v741_note"]="LAB: set totals 6.5+, player games, tie-breaks, both win a set; separate frozen validation."
        write(META,meta)
    print(json.dumps({"market_lab_v741_ready":ready},ensure_ascii=False))
