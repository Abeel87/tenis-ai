...
def _requested_bookmaker_payload(row: dict):
    ...
    OddsPapi exposes separate identities such as superbet, superbet.pl and
    superbet.ro. Generic/foreign Superbet must never satisfy the Polish
    operator contract.
    """
...
def _direct_fixture_offer_priority(row: dict):
    """Order request spending only; never filter fixture availability."""
    text = " ".join(
        str(row.get(key) or "")
    ...
    grand_slam = any(
        token in text
        for token in (
            "grand slam", "us open", "australian open",
...
def _direct_offer_due(cache_entry: dict | None, stage: str, now: datetime):
    ...
    if cache_entry.get("bookmaker_key") != BOOKMAKER:
        # Legacy cache predates strict sportsbook identity: one clean recheck.
        return True
...
def refresh_availability(results: list[dict], now=None):
    ...
    try:
        ...
        odds_rows = []
        if tournament_ids:
            # Cheap first pass only. Tournament batches are not current-offer
            # authority because they may contain historical tournament rows.
            time.sleep(1.05)
        ...
        for fixture_id, discovered in sorted(
            ...
            if due:
                ...
                if total_budget_left and direct_budget_left and refresh_budget_left:
                    ...
                    quota["direct_fixture_requests_used"] = int(
                        quota.get("direct_fixture_requests_used") or 0
                    ) + 1
                    try:
                        payload = _request(
                            "odds",
                        ...
                        for direct_row in rows:
                            if not isinstance(direct_row, dict):
                                continue
                            row_id = str(direct_row.get("fixtureId") or "")
                            same, _kind = _same_discovered_fixture(discovered, direct_row)
                            if not same or (row_id and row_id != fixture_id):
                                continue
                            start = _parse_dt(direct_row.get("startTime"))
                            if start is None or not (
                                date_from <= start.date().isoformat() <= date_to
                            ):
                                continue
                            bookmaker_odds = direct_row.get("bookmakerOdds")
                            bookmaker_keys = (
                                set(bookmaker_odds.keys())
                                if isinstance(bookmaker_odds, dict)
                                else set()
                            )
                            operator_bookmakers_seen.update(
                                str(key) for key in bookmaker_keys
                            )
                            ...
                            item = _sanitize_fixture(direct_row, market_meta)
                            if item:
                                offer = dict(item)
