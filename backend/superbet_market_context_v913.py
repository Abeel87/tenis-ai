...
normalization contract used by the Superbet PLAYABLE layer:
...
def batched_request(
    ...
    **params,
    ...
    """Call v9.1 normally, except tournament odds are batched to <= 5 IDs."""
...
def _winner_pick(outcome_name, bookmaker_outcome_id, p1, p2):
    """Resolve catalogue 1/2/X before looking at bookmaker-specific labels."""
    outcome = base._norm(outcome_name)
    if outcome in {"1", "p1", "participant 1", "player 1"}:
        return p1
    if outcome in {"2", "p2", "participant 2", "player 2"}:
        return p2
    if outcome in {"x", "draw", "tie"}:
        return "draw"
    ...
    if bookmaker in {"2", "p2", "participant 2", "player 2", "away"}:
        return p2
    if bookmaker in {"x", "draw", "tie"}:
        return "draw"

    n1, n2 = base._norm(p1), base._norm(p2)
    if n1 and (n1 in outcome or n1 in bookmaker):
        return p1
    if n2 and (n2 in outcome or n2 in bookmaker):
        return p2
        ...
        for outcome_id, outcome_data in (market_data.get("outcomes") or {}).items():
            ...
            for player_data in (outcome_data.get("players") or {}).values():
                ...
                player = (
                    ...
                    if player_side == "p2"
                    else player_data.get("playerName")
                )
                selection = {
                    "market": canonical,
                    "pick": pick,
...
def prepare() -> dict:
    ...
    original_sanitize_fixture = base._sanitize_fixture
    original_availability_due = base._availability_due
    original_refresh_hours = base.REFRESH_HOURS
    original_monthly_cap = base.MONTHLY_REQUEST_CAP

    previous = base._read(base.AVAILABILITY, {})
    force_parser_refresh = not isinstance(previous, dict) or previous.get("runtime_adapter_version") != VERSION
    ...
    base._sanitize_fixture = _sanitize_fixture
    if force_parser_refresh:
        base._availability_due = lambda _previous, _now: True
