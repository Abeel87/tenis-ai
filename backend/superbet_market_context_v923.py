...
"""Tenis AI v9.2.3 — compact raw Superbet market-family audit.

This is an instrumentation wrapper around v9.1.5. It does not perform any
additional HTTP/API call. It only inspects the raw market IDs already present
in the normal OddsPapi `odds-by-tournaments` response and the already-cached
market catalogue, then persists compact family metadata with all prices and
bookmaker odds omitted.

Purpose: before requesting any extra tennis data or training a new target, show
exactly which real Superbet market families are still unmapped.
"""
...
def _superbet_markets(row: dict) -> dict:
    ...
    if not isinstance(book, dict):
        book = next(
            (
                value
                for key, value in bookmaker_odds.items()
...
def _raw_families(row: dict, meta: dict) -> list[dict]:
    """Collapse active market IDs to unique semantic families; discard odds."""
    ...
    for market_id, market_data in _superbet_markets(row).items():
        ...
        if checkpoint is not None:
            family["checkpoints"].add(int(checkpoint))
        if player_side:
            family["player_sides"].add(str(player_side))
        handicap = _num(market_meta.get("handicap"))
        if handicap is not None and len(family["handicaps"]) < MAX_HANDICAP_SAMPLES:
            family["handicaps"].add(handicap)
        if len(family["sample_market_ids"]) < MAX_MARKET_ID_SAMPLES:
            family["sample_market_ids"].append(str(market_id))
...
def _sanitize_with_audit(row: dict, meta: dict, original_sanitize):
...
def build_audit(fixtures) -> dict:
        ...
        for item in families:
            ...
            row = aggregate.setdefault(
                key,
                {
                    ...
                    "player_prop": item.get("player_prop"),
            ...
            for value in item.get("handicaps") or []:
                if len(row["handicaps"]) < MAX_HANDICAP_SAMPLES:
                    ...
                    if number is not None:
                        row["handicaps"].add(number)
            for value in item.get("checkpoints") or []:
                try:
                    row["checkpoints"].add(int(value))
                except (TypeError, ValueError):
                    pass
            for value in item.get("sample_market_ids") or []:
                token = str(value)
                if token not in row["sample_market_ids"] and len(row["sample_market_ids"]) < MAX_MARKET_ID_SAMPLES:
                    row["sample_market_ids"].append(token)

    families = []
    for item in aggregate.values():
        out = dict(item)
        out["handicaps"] = sorted(item["handicaps"])
        out["checkpoints"] = sorted(item["checkpoints"])
        families.append(out)
    ...
    return {
        "version": VERSION,
        "fixtures_with_family_audit": fixture_count,
        "unique_raw_market_families": len(families),
        "unique_unrecognized_market_families": len(unrecognized),
        "families": families,
        "unrecognized_families": unrecognized,
        "additional_external_requests": 0,
        "prices_used": False,
        "contract": {
            "uses_existing_odds_response_only": True,
            "stores_prices": False,
            "does_not_request_extra_tennis_data": True,
        },
    }


def _stamp_audit() -> dict:
    ...
    availability["raw_family_audit_version"] = VERSION
...
def prepare() -> dict:
    original_sanitize = v913._sanitize_fixture
    original_version = v913.VERSION

    def audited_sanitize(row: dict, meta: dict):
        return _sanitize_with_audit(row, meta, original_sanitize)

    # Changing the runtime adapter version forces exactly one normal parser
    # refresh after rollout, so old cached fixtures gain family metadata. It
    # does not add another endpoint or per-match request.
    v913._sanitize_fixture = audited_sanitize
    v913.VERSION = VERSION
    try:
        result = dict(v913.prepare())
    finally:
        v913._sanitize_fixture = original_sanitize
        v913.VERSION = original_version

    audit = _stamp_audit()
    result["raw_family_audit_version"] = VERSION
