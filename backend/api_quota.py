...
UA = "TenisAI-CentralQuota/1.0"
REPORT_VERSION = "v8.3B"
CANONICAL_MODULE = "backend/api_quota.py"
...
def request_interval_seconds(per_minute: int | None, utilization: float = 0.90) -> float:
    """Minimum spacing for one sequential caller while keeping provider headroom."""
    try:
        rpm = max(1, int(per_minute or 60))
    except (TypeError, ValueError):
        rpm = 60
    try:
        util = max(0.10, min(0.95, float(utilization)))
    except (TypeError, ValueError):
        util = 0.90
    return 60.0 / (rpm * util)
