...
async function upsertEvaluation(supabase: any, experiment: Json, evaluation: Json) {
  ...
  const status = protectedStatus ? current.status : evaluation.status;
  const row = {
  ...
  const evaluatedAt = snapshot.timestamp || new Date().toISOString();
  ...
  if (newlyQualified) {
    const emailPayload = {
      ...
      odds: snapshot.raw_odds,
      stake: snapshot.final_stake,
      bankroll: snapshot.bankroll_before,
      net_ev: snapshot.expected_value_net,
...
async function staffEmails(supabase: any): Promise<string[]> {
  ...
  const configured = (Deno.env.get("INEED_ALERT_EMAIL") || "")
    .split(/[;,]/)
...
async function flushEmails(supabase: any) {
  ...
  const gmailUser = firstEnv(["INEED_GMAIL_USER", "INEED_GMAIL_EMAIL", "GMAIL_USER", "GMAIL_EMAIL", "SMTP_USER"]) || recipients[0];
  const gmailAppPassword = firstEnv(["INEED_GMAIL_APP_PASSWORD", "GMAIL_APP_PASSWORD", "GOOGLE_APP_PASSWORD", "SMTP_APP_PASSWORD", "SMTP_PASSWORD"])?.replace(/\s+/g, "");
  const configState = {
  ...
  if (!gmailUser || !gmailAppPassword || !recipients.length) {
    const missing = [
      !gmailUser ? "sender" : null,
      !gmailAppPassword ? "app_password" : null,
      !recipients.length ? "recipients" : null,
    ].filter(Boolean).join(",");
    for (const row of rows) {
      await supabase.from("ineed_email_events").update({
        status: "PENDING_CONFIG",
        last_error: `Missing Gmail SMTP config: ${missing}`,
        attempt_count: Number(row.attempt_count || 0) + 1,
      }).eq("id", row.id);
    }
    return { sent: 0, pending: rows.length, configuration_required: true, provider: "gmail-smtp", ...configState };
  }
  ...
  for (const row of rows) {
    ...
    const html = `<h2>💰 iNeed$ znalazł value</h2><p><b>${p.match || "Mecz"}</b></p><p>Typ: ${p.selection || p.market || "—"}<br>Superbet: ${p.odds ?? "—"}<br>Proponowana stawka: ${p.stake ?? "—"} PLN<br>Bankroll: ${p.bankroll ?? "—"} PLN<br>Net EV: ${p.net_ev == null ? "—" : (Number(p.net_ev)*100).toFixed(1)+"%"}<br>Confidence: ${p.confidence ?? "—"}<br>Risk: ${p.risk || "—"}</p><p><a href="${appUrl}">OTWÓRZ MECZ W TENIS AI</a></p>`;
    const text = [
      "iNeed$ znalazł value",
      p.match || "Mecz",
      `Typ: ${p.selection || p.market || "—"}`,
      `Superbet: ${p.odds ?? "—"}`,
      `Proponowana stawka: ${p.stake ?? "—"} PLN`,
      `Bankroll: ${p.bankroll ?? "—"} PLN`,
      `Net EV: ${p.net_ev == null ? "—" : (Number(p.net_ev)*100).toFixed(1)+"%"}`,
    ...
    try {
      const info = await transport.sendMail({
        from: `"Tenis AI iNeed$" <${gmailUser}>`,
        to: recipients,
        subject: "💰 iNeed$ znalazł value",
        text,
        html,
      });
      await supabase.from("ineed_email_events").update({
  ...
  return { sent, pending: rows.length - sent, provider: "gmail-smtp", ...configState };
}
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { error: healthError } = await supabase.from("ineed_runtime_health").upsert({ experiment_id: state.experiment.id, status: "OK", last_sync_at: payload.generated_at || new Date().toISOString(), detail: { ...(payload.health || {}), settlement_results: settlementResults.length, evaluation_results: evaluationResults.length, email }, updated_at: new Date().toISOString() });
