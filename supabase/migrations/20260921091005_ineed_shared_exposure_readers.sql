-- LOGIC-12: move V1 placement exposure reads to the normalized shared DB source.
...
begin
  ...
  select coalesce(sum(r.stake),0) into total_exposure from public.ineed_open_risk_exposures r where r.experiment_id=e.id;
  select coalesce(sum(r.stake),0) into match_exposure from public.ineed_open_risk_exposures r where r.experiment_id=e.id and r.match_id=s.match_id;
  select coalesce(sum(r.stake),0) into market_exposure from public.ineed_open_risk_exposures r where r.experiment_id=e.id and exists (select 1 from unnest(r.markets) market_name where trim(market_name)=trim(s.market));
  select coalesce(sum(r.stake),0) into player_exposure from public.ineed_open_risk_exposures r where r.experiment_id=e.id and exists (select 1 from unnest(r.players) player_name where lower(trim(player_name)) in (p1,p2));
