...
The post-read-model atomicity audit freezes the future database ownership boundary without implementing it. Current V1 placement and settlement already serialize bankroll mutation by locking the same `ineed_experiments` row `FOR UPDATE`; any future builder reservation must use that same experiment lock. The minimal future design uses one `ineed_builder_tickets` durable owner, the existing `ineed_bankroll_ledger` as the only ledger, and one normalized `ineed_open_risk_exposures` DB source spanning V1 open bets plus builder `SHADOW_RESERVED` owners.

A builder reserve path cannot be enabled while V1 placement or `ineed-sync::activeState()` reads exposure only from `ineed_shadow_bets`. Before runtime reservation, both must consume the shared DB exposure source with exact V1-only equivalence. Builder persistence must remain outside `open_bets`; current Phase-5 tickets remain non-runtime/nonpersistent until a separately authorized promotion.
