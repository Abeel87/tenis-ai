...
- **Prediction Ledger SHADOW** ? canonical prospective evidence owner `backend/prediction_ledger_shadow.py`; captures the first stable pre-match Current/CatBoost/TabPFN prediction per supported candidate after AutoLearn decoration but before generator/PLAYABLE/Symphony/iNeed$ selection. Phase-1 is append-only, `ALL`-labeled, zero-consumer SHADOW evidence; downstream selection facts remain N/D until joined from their canonical owners.
