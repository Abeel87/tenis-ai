...
The next bounded read-side migration targets `ineed_admin_start_experiment()`: its previous-experiment open-exposure rollover guard is changed from V1-only `ineed_shadow_bets` to `ineed_open_risk_exposures`, without changing admin auth, locking, final-bankroll/config rollover or experiment initialization. After that, two pre-writer consumers remain separate: V1 settlement `other_exposure` bookkeeping and `frontend/ineed.js` exposure presentation. No builder reserve writer is enabled by these read-side migrations.
