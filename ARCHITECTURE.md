...
Production still has no deployed builder writer/RPC. The repository now contains a separate fail-closed SHADOW writer migration, `20260921123000_ineed_builder_reservation_writer_shadow.sql`, but this branch does not deploy it and adds no runtime caller. The RPC uses the same experiment-row `FOR UPDATE` mutex, validates the immutable Phase-5 ticket, revalidates only the frozen stake against `ineed_open_risk_exposures`, and requires an explicit experiment `builder_reservation` enable flag/version. `ineed_system_place_bet()` and live `ineed-sync::activeState()` continue to use the shared view while preserving V1-only `open_bets` for settlement. Production Edge remains v11.
...
`TENIS_AI_LOGIC12_DEPLOYMENT_READINESS_AUDIT.md` records the shared-read deployment closeout, and `TENIS_AI_LOGIC12_BUILDER_RESERVATION_WRITER_AUDIT.md` records the later source-only writer boundary. The existing iNeed$ email path (`ineed_email_events` + `QUALIFIED` Gmail SMTP delivery in `ineed-sync`) remains intact. The new writer is repository-only, service-role-only and disabled by absent experiment config; there is still no runtime caller or builder settlement, and production has not received this writer migration.
