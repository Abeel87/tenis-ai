...
Caller enablement is blocked upstream, not at the RPC boundary. Current `superbet_direct_current.json` excludes combination/BetBuilder rows by design, and the exact pre-priced/dynamic combined-quote parsers are not wired into any production refresh or runner. The next architectural unit is therefore a separate read-only exact builder-quote artifact owned by the Superbet refresh path. Only after that artifact exists may the SHADOW scoped runner become the canonical pure Phase-4/5 ticket producer; reservation RPC wiring and `builder_reservation.enabled=true` remain later, separate gates.
