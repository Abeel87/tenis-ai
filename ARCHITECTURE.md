...
The deployed schema still has no builder writer/RPC. `ineed_system_place_bet()` reads total/match/player/market exposure from `ineed_open_risk_exposures`, and live `ineed-sync::activeState()` reads the same view into `risk_exposures` for exposure/equity while preserving V1-only `open_bets` for settlement. Production Edge is now v11 after the controlled 2026-09-21 rollout.
...
`TENIS_AI_LOGIC12_DEPLOYMENT_READINESS_AUDIT.md` now records the deployment closeout evidence. The existing iNeed$ email path (`ineed_email_events` + `QUALIFIED` Gmail SMTP delivery in `ineed-sync`) remained intact: 36 historical live events are SENT with provider IDs and zero unsent events remained after rollout. Shared read-side deployment does not authorize builder reservation writes; no builder reserve RPC/caller exists and builder settlement is still `NOT_IMPLEMENTED`.
