...
The dormant schema has no writer/RPC and grants no builder-table write path. Current V1 placement and `ineed-sync::activeState()` still read exposure only from `ineed_shadow_bets`, so builder reservation remains impossible. Before runtime reservation, both readers must migrate to the shared DB exposure source with exact V1-only equivalence. Builder persistence remains outside `open_bets`; current Phase-5 tickets remain non-runtime/nonpersistent until a separately authorized promotion.
