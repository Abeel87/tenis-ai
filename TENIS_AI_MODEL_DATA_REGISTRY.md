...
**Status:** ACTIVE - phases 1-6, shared-exposure read/accounting/presentation, and the dormant SHADOW reservation writer are merged and deployed through PR #452 + production Edge v11. The writer remains disabled by absent experiment config and has no runtime caller; one-ticket builder settlement remains `NOT_IMPLEMENTED`, and real-money execution remains disabled.
...
**Caller/enablement audit:** PR #453 deployment closeout is merged; live writer stays disabled. Audit proves the actual upstream blocker is not the RPC but quote/ticket production: Phase-4/5 builder functions have no runtime caller, current `superbet_direct_current.json` intentionally contains only single-market selections, and `parse_event_payload()` skips BetBuilder/combination rows. Exact pre-priced/dynamic builder quote parsers exist but are test-only. The next owner is therefore an additive read-only exact-builder-quote artifact in the canonical Superbet refresh path; no reservation caller/flag enablement is allowed before that artifact and a separate pure Phase-4/5 runtime producer are independently proven.
