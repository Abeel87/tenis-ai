...
**Frontend shared-exposure candidate:** `supabase/migrations/20260921101354_ineed_staff_exposure_summary.sql` adds a staff-authorized aggregate RPC over service-only `ineed_open_risk_exposures`; it does not expose row-level builder/V1 risk data to browser clients. `frontend/ineed.js` uses that aggregate for exposure/equity/open-unit presentation and accepts V1 fallback only when the RPC is absent (`PGRST202` / `42883`) before deployment; other failures remain fail-closed. Local proof before final fresh-main gate: focused 25/25, iNeed 133/133, full repo 1466/1466, 500-state fallback equivalence, SQL parse 3 statements, JS syntax GREEN, UI 377/82 PASS, Project Health 0 FAIL / 1 WARN.

**Pre-writer deployment boundary:** after this candidate there are no known repository-side V1-only read/presentation consumers. Builder reserve runtime is still blocked until a separate controlled deployment/readiness phase applies and verifies the dormant shared-exposure migrations/reader changes and candidate Edge source. Builder settlement and real-money execution remain disabled.
