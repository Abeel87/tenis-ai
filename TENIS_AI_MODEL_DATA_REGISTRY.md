...
**Lifecycle blocker:** `BLOCKED_BY_OPERATOR_SETTLEMENT_EVIDENCE`. Current market-238733/public quote evidence proves exact pricing identity but not a terminal one-ticket result or payout. Completed sampled public event payloads expose no retained `odds` / `oddsResults` builder result. The tennis-specific Superbet communication effective 2026-07-31 states that voided Bet Builder selections do not cancel the whole ticket and that the total odds are recalculated from the remaining valid selections, including retirement/disqualification cases. This narrows the lifecycle contract but still does not prove the exact numeric reprice for a one-leg remainder or expose the final whole-ticket payout/result, so code must not infer either value.
...
**Early exact-snapshot delivery (2026-09-25):** Main's Superbet refresh can replace `results.json` during the hour-long Player DNA backtests. The existing pre-push digest guard correctly rejects that stale sidecar, but late-only delivery leaves `meta.semantic_readiness_shadow` at N/D indefinitely. The same Player DNA producer now commits only its already validated exact-snapshot `readiness_engine_shadow.json` and additive `meta.semantic_readiness_shadow` immediately after the LOGIC-08 isolation/digest guard and before heavy backtests; a fresh `origin/main` digest is checked before every push. Successful hourly Superbet workflow completion runs this short portion without launching the long backtests. Long Player DNA reports keep their independent SHADOW pipeline; stale long-report publication is skipped, never rebased onto changed results. Core and browser remain fail closed on a subsequent mismatched snapshot. No changed model math, market gate, iNeed$ decision or real betting.\n\n**Post-DNA delivery repair (2026-09-25):** The canonical `readiness_engine_shadow.project_observability_meta()` projects only validated exact-snapshot, isolation-safe SHADOW evidence after the Player DNA workflow guard. The Player DNA publisher includes `meta.json` with its sidecar and compares the run-local `results.json` digest to `origin/main` immediately before every rebase/push; a changed result aborts stale publication. Core `backend/update.py` continues to fail closed while it sees the prior sidecar. Existing `meta.updated_at`, legacy `model_ready`, probabilities and all runtime/learning gates remain untouched. No browser-side computation or alternate readiness owner is added.
...
**Training archive budget change (2026-09-25, production verified):** The canonical `training_archive_enforce_budget` and `training_archive_enforce_object_budget` remain the only write guards. Raise only their archive reservation cap from 250,000,000 to 300,000,000 B. Preserve the independent 750,000,000 B project accounted-storage ceiling, physical-plus-missing-reservation accounting, SHA dedupe, advisory lock, ACLs, latest-two manifest retention, pins and 168h grace. The Free organization has a 1 GB Storage quota; measured physical project objects were 561,343,745 B, including 249,998,501 B archive, before this change. This budget adjustment does not alter model/training, runtime-private, iNeed$ or automatic betting. PR #511 merged as `143a67b0adae334a0b91b591c836b0e1c24b0c8b` after 7/7 green workflows. Production migration `training_archive_budget_300mb` is applied. Inventory rerun #36083363734 attempt 2 completed GREEN: manifest `complete` at 10:53:46Z with 10,521/10,521 entries. Archive reserved 276,887,581 B and project physical Storage 588,232,825 B. Continue GC audits naturally.
