...
**Status:** IN PROGRESS — phase-1/2/3/4 audit complete; exact-snapshot SHADOW sidecar delivery merged in PR #410 (`57a49ade723179417cba07e77c791b1e425f38ca`) and post-merge publication verified. Active substep: R12/R13 additive meta observability only.
...
The first selected observability path is R12/R13: additive SHADOW-labeled aggregate/meta telemetry beside unchanged legacy `meta.model_ready` in `backend/update.py`. Exact snapshot delivery is now proven; the active implementation consumes only an already-published sidecar through the canonical projection helper, after `results.json` is written. Stale/missing/misaligned/non-isolated evidence is N/D, never zero. Diagnostic/admin UI counters are second; moderator filtering is explicitly not observability-only. Legacy `model_ready` cannot be removed while any audited consumer still depends on it.
...
The published current sidecar is exact-snapshot aligned for 109 matches at digest `79645fe3f81bc628b7152f1597811a09f1d5dc3c29bce3bbde2c0a15f6a7d3bd`, with history and Player State alignment true, zero legacy-reference mismatches, `network_calls=0` and all isolation flags false. R12/R13 therefore have the required delivery evidence. Their implementation is telemetry-only: `backend/update.py` may project this already-published evidence into `meta.semantic_readiness_shadow`, while `meta.model_ready` and every runtime/learning gate remain unchanged. UI exposure remains a later observability step.
