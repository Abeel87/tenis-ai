...
function label(text,kind){
  ...
  el.className='p751-note v950-detail-label';
  el.dataset.v950Label=kind;
...
function removeOldLabels(root){
  root.querySelectorAll('[data-v950-label]').forEach(el=>el.remove());
}
...
function organize(){
  ...
  const matchup=root.querySelector('.p751-matchup');
  const decision=root.querySelector('.dc87');
  ...
  if(verdict){
    ...
    verdict.querySelectorAll('article').forEach(article=>{
      const title=article.querySelector('span');
      if(title&&/^Najlepszy typ$/i.test(title.textContent.trim()))title.textContent='Najlepszy sygnał modelu';
      if(title&&/^Alternatywa$/i.test(title.textContent.trim()))title.textContent='Alternatywa modelu';
    });
    verdict.before(label('2 · MODEL / RAW — analiza niezależna od dostępności Superbet','model'));
  }
  ...
  root.querySelectorAll('#pi85-detail,.pi851-detail,[data-pi85-detail]').forEach(pi=>{
    ...
    if(header&&!/SHADOW|kontekst/i.test(header.textContent))header.textContent=`${header.textContent} · SHADOW / kontekst`;
  });
...
let timer=0;
function schedule(ms=0){clearTimeout(timer);timer=setTimeout(organize,ms)}

function boot(){
  schedule(0);
  document.addEventListener('click',event=>{
    if(event.target?.closest?.('[data-p751-open],[data-v917-top]'))schedule(40);
  },true);
  if('MutationObserver'in window){
    const overlay=document.querySelector('#p751-match-overlay')||document.body;
    new MutationObserver(()=>schedule(30)).observe(overlay,{childList:true,subtree:true});
  }
}
