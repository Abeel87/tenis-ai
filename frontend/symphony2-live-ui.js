...
(() => {
  ...
  async function ensureDetail(force=false){const host=document.querySelector('#app[data-match-key]');if(!host)return;if(window.TENIS_AI_SYMPHONY2?.renderMatchDetail){try{await window.TENIS_AI_SYMPHONY2.renderMatchDetail(force)}catch{}if(host.querySelector('#symphony2-match-detail'))return}const data=await fetchFeed(force);if(!data)return;const row=rowForDetail(data);if(!row)return;const match=projectMatchFor(row,host?.dataset?.matchKey||'');const screen=host.querySelector('.p751-detail-screen')||host;screen.querySelector('#symphony2-match-detail')?.remove();const wrap=document.createElement('div');wrap.innerHTML=detailFallbackHtml(row,data,match);const block=wrap.firstElementChild;const decision=screen.querySelector('.dc87');decision?decision.insertAdjacentElement('beforebegin',block):screen.append(block)}
