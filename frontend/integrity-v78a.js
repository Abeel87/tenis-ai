/* Tenis AI v7.8A — Integrity Guard */
(() => {
  async function boot(){
    try{
      ...
      el.textContent=d.status==='PASS'
        ?`✓ INTEGRITY PASS · ${d.matches||0}`
        :`⚠ INTEGRITY FAIL · ${d.hard_errors||0}`;
      el.title=d.status==='PASS'
        ?'Automatyczne testy spójności danych przeszły.'
        :'Wykryto twardy błąd spójności danych.';
      host.appendChild(el);
    }catch{}
  }
  setTimeout(boot,500);
