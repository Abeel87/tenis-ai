...
(() => {
  ...
  async function enhanceStats(){const host=document.querySelector('#pi85-stats');if(!host||host.querySelector('.pi851-stats-extra'))return;const t=await telemetry(),pi=t?.player_intelligence_v85;if(!pi)return;host.classList.add('pi851-enhanced');const n=Number(pi.settled_rows||0),m=pi.models||{},progress=Math.min(100,n/5*100),models=[['Player',m.player],['Ensemble',m.ensemble],['Ensemble + Player',m.ensemble_player_shadow],['Generator proxy + Player',m.generator_player_shadow]].filter(([,x])=>x);host.insertAdjacentHTML('beforeend',`<div class="pi851-stats-extra"><section class="pi851-learning"><div class="pi851-learning-title"><div><span>🧬 Stan nauki Player Intelligence</span><b>${n?`${n} rozliczonych`:'Czeka na pierwsze rozliczenia'}</b></div><em>SHADOW</em></div><div class="pi851-learning-grid"><span><small>Profile aktywne teraz</small><b>${activeProfiles()}</b></span><span><small>Rozliczone HIT/MISS</small><b>${n}</b></span><span><small>Próg śledzenia</small><b>65+</b></span><span><small>Wykres trendu</small><b>od 5</b></span></div><div class="pi851-progress"><i style="width:${progress}%"></i></div><p>${n?'Liczymy wyłącznie prognozy zamrożone przed meczem.':'<b>0 nie oznacza 0% skuteczności.</b> Nie ma jeszcze rozliczonego sygnału v8.5.'}</p></section><section class="pi851-metrics"><header><b>Dokładne metryki</b><span>n · ≥65 · ACC · Brier · LogLoss</span></header><div class="pi851-metrics-table">${models.map(([name,x])=>`<div><b>${esc(name)}</b><span>n ${Number(x.n||0)}</span><span>≥65 ${Number(x.selected_n||0)}</span><span>ACC ${val(x,'accuracy','pct')}</span><span>B ${val(x,'brier','dec')}</span><span>LL ${val(x,'log_loss','dec')}</span></div>`).join('')}</div></section><section class="pi851-surfaces"><header><b>Nawierzchnie</b><span>Ensemble + Player</span></header><div>${Object.keys(pi.by_surface||{}).length?Object.entries(pi.by_surface).map(([s,x])=>{const z=x?.ensemble_player_shadow||{};return `<span><b>${esc(s.toUpperCase())}</b><em>${val(z,'accuracy','pct')}</em><small>n=${Number(z.selected_n||0)} · B ${val(z,'brier','dec')}</small></span>`}).join(''):'<p>Pojawią się po pierwszych rozliczeniach.</p>'}</div></section></div>`)}
