...
(() => {
  ...
  async function decorateCards(force=false){const cards=[...document.querySelectorAll('.p751-match-card[data-p751-open]')];if(!cards.length)return;const data=await fetchFeed(force);if(!data)return;for(const card of cards){card.querySelector('[data-s2-live-card]')?.remove();const row=rowForKey(data,decodeKey(card));if(!row)continue;const footer=card.querySelector('footer');const wrap=document.createElement('div');wrap.innerHTML=badgeHtml(row);const badge=wrap.firstElementChild;footer?footer.insertAdjacentElement('beforebegin',badge):card.append(badge)}}
  ...
  function openScenarios(){const api=window.TENIS_AI_SCENARIOS;if(api?.open){api.open('home');return true}return false}

  document.addEventListener('click',async e=>{
    const btn=e.target?.closest?.('[data-sc-generate]');if(!btn||btn.dataset.s2Retry==='1')return;
    const guard=window.TENIS_AI_GENERATOR_QUALITY_V888;if(!guard?.ready)return;
    e.preventDefault();e.stopPropagation();e.stopImmediatePropagation?.();const oldText=btn.textContent;btn.disabled=true;btn.textContent='⏳ Przygotowuję modele…';
    const result=await Promise.race([Promise.resolve(guard.ready).then(()=>({ready:true})),timeout(2500)]);
    if(result?.timeout){window.TENIS_AI_GENERATOR_QUALITY_V888=Object.freeze({version:`${guard.version||'v8.8.9'}+timeout-safe`,ready:Promise.resolve(),checkGroup:guard.checkGroup,reviewGenerated:guard.reviewGenerated,currentProblems:guard.currentProblems});console.warn('[Scenario] Quality auxiliary fetch timed out; continuing with existing structural quality guard.')}
  ...
  let timer=null;const observer=new MutationObserver(()=>{clearTimeout(timer);timer=setTimeout(()=>{decorateCards(false);ensureDetail(false)},90)});observer.observe(document.documentElement,{subtree:true,childList:true,attributes:true,attributeFilter:['hidden']});[0,250,900,1800].forEach((ms,i)=>setTimeout(()=>decorateCards(i===0),ms));setTimeout(()=>ensureDetail(false),300);
