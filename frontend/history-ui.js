...
function symPredictions(entries){const entry=latestSym(entries);if(!entry)return [];return (entry.selection||[]).filter(s=>s&&s.market).map(s=>({signal:s,source:'Symfonia 2.0',unit:'%',value:number(s.probability??s.operator_model_probability),result:status(s.result),playable:s.fixture_line_verified===true,odds:oddsOf(s),capturedAt:entry.captured_at||null,kind:'symphony',predictionId:entry.prediction_id,jointProbability:number(entry.joint_probability),compositionResult:status(entry.result)}));}
...
function sourceBadge(p){return `<span class="badge ${p.kind==='symphony'?'ok':''}">${E(p.source)}</span>${p.playable?'<span class="badge ok">PLAYABLE / linia zweryfikowana</span>':''}`;}
