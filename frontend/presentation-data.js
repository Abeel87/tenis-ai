...
const api=window.TENIS_AI_PLAYABLE_UI_V917;
if(feed?.operator!=='superbet.pl'||feed.status!=='OK'||feed.prices_used!==false||!api.active(m)||m.superbet_market_v91?.operator!=='superbet.pl'||!api.availability(m).has(api.signature(s)))return null;
...
const r=rows[0],priceContext={...m,superbet_market_v91:{...m.superbet_market_v91,source_generated_at:feed.generated_at,operator_start_time:r.operator_start_time}};
if(!api.active(priceContext)||!Number.isFinite(Date.parse(r.operator_start_time)))return null;
try{const url=new URL(r.event_url);if(url.protocol!=='https:'||url.hostname!=='superbet.pl')return null}catch{return null}
const matches=(Array.isArray(r.canonical_selections)?r.canonical_selections:[]).filter(x=>x.operator==='superbet.pl'&&x.operator_available===true&&x.operator_price_verified===true&&x.prices_used===false&&x.operator_selection_status!=='suspended'&&(!x.operator_selection_status||x.operator_selection_status==='active')&&api.signature(x)===api.signature(s)&&num(x.set_no)===num(s.set_no??(/^set([123])_/.exec(api.canonicalMarket(s.market))?.[1]))&&name(x.player)===name(s.player)&&api.isPlayable(priceContext,x));
