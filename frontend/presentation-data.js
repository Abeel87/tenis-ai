...
const cache=new Map();
async function json(path,force=false){
 const immutable=/^data\/delivery\/matches\/[a-f0-9]{64}\.json$/.test(path);
 const ttl=immutable?Infinity:/(?:current|index\.json|symphony\.json|meta\.json)/.test(path)?60000:300000;
 const prior=cache.get(path);if(force||prior&&Date.now()-prior.at>=ttl)cache.delete(path);
 if(!cache.has(path)){
  const entry={at:Date.now()};
  entry.promise=fetch(path,{cache:'no-store'}).then(r=>{if(!r.ok)throw Error('Nie można pobrać danych ('+r.status+').');return r.json()}).catch(e=>{if(cache.get(path)===entry)cache.delete(path);throw e});
...
function filterRows(rows,f,{saved=new Set(),events=()=>[],playable=()=>false,now=Date.now()}={}){return rows.filter(m=>{const t=Date.parse(m.scheduled_time);if(f.tour&&f.tour!=='all'&&String(m.tour||'').toLowerCase()!==f.tour)return false;if(f.surface&&f.surface!=='all'&&m.surface!==f.surface)return false;if(f.query&&!norm(m.p1+' '+m.p2+' '+m.tournament).includes(norm(f.query)))return false;if(f.focus==='soon'&&!(t>now&&t<=now+7200000))return false;if(f.focus==='today'){const day=new Date(t).toLocaleDateString('en-CA',{timeZone:'Europe/Warsaw'}),today=new Date(now).toLocaleDateString('en-CA',{timeZone:'Europe/Warsaw'});if(day!==today)return false;const timeApi=window.TENIS_AI_MATCH_TIME;if(timeApi?.isCurrent&&!timeApi.isCurrent(m,now))return false;}if(f.focus==='saved'&&!saved.has(key(m)))return false;if(f.focus==='playable'&&!playable(m))return false;if(f.focus==='data'&&!events(m).some(x=>x.value!=null))return false;return true}).sort((a,b)=>f.sort==='name'?String(a.p1).localeCompare(String(b.p1),'pl'):f.sort==='quality'||f.focus==='top'?(b.delivery_summary?b.ui_summary?.top?.v??-1:window.TENIS_AI_MODEL_API.signals(b,1)[0]?.v??-1)-(a.delivery_summary?a.ui_summary?.top?.v??-1:window.TENIS_AI_MODEL_API.signals(a,1)[0]?.v??-1):(Date.parse(a.scheduled_time)||Infinity)-(Date.parse(b.scheduled_time)||Infinity));}
