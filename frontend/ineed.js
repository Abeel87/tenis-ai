...
async function load(){
  ...
  const [s,b,l,h,r]=await Promise.all([
    ...
    c.rpc('ineed_staff_exposure_summary',{target_experiment_id:exp.id})
  ]);
  for(const x of [s,b,l,h])if(x.error)throw x.error;
  const bets=b.data||[],v1Open=bets.filter(x=>['PENDING','SHADOW_PLACED'].includes(x.status));
  let risk;
  if(r.error){
...
function historyGroup(title,rows,card){
  return `<details class="ineed-history-group"><summary><span>${esc(title)}</span><b>${rows.length}</b></summary><div class="ineed-history-content">${rows.length?rows.slice(0,5).map(card).join(''):'<div class="ineed-empty">Brak pozycji w tej grupie.</div>'}${rows.length>5?`<details class="ineed-older"><summary>Starsze pozycje (${rows.length-5})</summary><div class="ineed-signals">${rows.slice(5).map(card).join('')}</div></details>`:''}</div></details>`;
}
...
async function render(){
  ...
  try{
    ...
    const cfg=d.exp.config||{},settled=d.bets.filter(x=>['WIN','LOSS','VOID','CANCELLED','SETTLED'].includes(x.status));
    ...
    const lastSync=Date.parse(d.health?.last_sync_at||''),recent=d.signals.filter(x=>Number.isFinite(lastSync)&&Date.parse(x.updated_at)>=lastSync-60000);
    ...
    const syncSummary=d.health?.last_sync_at
      ? `Ostatnia synchronizacja: ${dt(d.health.last_sync_at)} · oceniono ${n(d.health.detail?.evaluations,recent.length)} · aktywne ${recent.filter(x=>['QUALIFIED','PENDING'].includes(x.status)).length}. ${reasons.length?'Odrzucenia: '+reasons.map(([code,count])=>esc(code)+' '+count).join(', ')+'.':'Brak odrzuceń w ostatnim odczycie.'} Mail: ${n(d.health.detail?.email?.sent)} wysłanych w ostatnim odczycie, ${n(d.health.detail?.email?.pending)} oczekujących.`
    ...
    const rejectedToday=rejected.filter(x=>dayKey(x.updated_at)===today),rejectedOlder=rejected.filter(x=>dayKey(x.updated_at)!==today);
    if(!current())return;
    app.innerHTML=`<div class="ineed-page"><div class="ineed-head"><div><div class="ineed-kicker">SHADOW · SUPERBET.PL</div><h1>💰 iNeed$</h1><div class="ineed-muted">${esc(d.exp.name)} · ${esc(d.exp.config_version)}</div></div><div class="ineed-actions">${admin()?'<button id="ineed-new" class="ineed-btn">Nowy eksperyment</button>':''}<a class="ineed-btn primary" href="#symphony">Symfonia 2.0</a></div></div><div class="ineed-grid"><div class="ineed-card"><span class="ineed-label">Bankroll</span><strong>${money(equity)}</strong><span class="${pl>=0?'ineed-good':'ineed-bad'}">${pl>=0?'+':''}${money(pl)}</span></div><div class="ineed-card"><span class="ineed-label">ROI</span><strong>${pct(roi)}</strong><span class="ineed-muted">Start ${money(d.exp.starting_bankroll)}</span></div><div class="ineed-card"><span class="ineed-label">Ekspozycja</span><strong>${money(exposure)}</strong><span class="ineed-muted">Dostępne ${money(available)}</span></div><div class="ineed-card"><span class="ineed-label">Drawdown</span><strong>${pct(dd)}</strong><span class="ineed-muted">Peak ${money(peak)}</span></div><div class="ineed-card"><span class="ineed-label">Sygnały dziś</span><strong>${todaySignals}</strong></div><div class="ineed-card"><span class="ineed-label">Otwarte</span><strong>${openUnits}</strong></div><div class="ineed-card"><span class="ineed-label">Rozliczone</span><strong>${settled.length}</strong></div><div class="ineed-card"><span class="ineed-label">Runtime</span><strong>${esc(d.health?.status||'—')}</strong><span class="ineed-muted">${dt(d.health?.last_sync_at)}</span></div></div><div class="ineed-section"><h2>Bankroll</h2><div class="ineed-card">${chart(d.ledger,d.exp.starting_bankroll)}</div></div><div class="ineed-section"><h2>Cel</h2>${milestones(equity)}</div><div class="ineed-section"><h2>Stan ostatniej oceny</h2><div class="ineed-card ineed-muted">${syncSummary}</div></div><div class="ineed-section"><h2>Aktywne sygnały · ${activeSignals.length}</h2><div class="ineed-signals">${activeSignals.slice(0,5).map(signalCard).join('')||'<div class="ineed-empty ineed-card">Brak aktywnych sygnałów. NO BET jest poprawnym wynikiem.</div>'}${activeSignals.length>5?`<details class="ineed-older"><summary>Pozostałe aktywne (${activeSignals.length-5})</summary><div class="ineed-signals">${activeSignals.slice(5).map(signalCard).join('')}</div></details>`:''}</div></div><div class="ineed-section"><h2>Historia zakładów SHADOW</h2><p class="ineed-muted">Grupy obejmują ostatnie ${d.bets.length} pobranych zakładów. Rozwiń grupę i pozycję, aby zobaczyć wszystkie szczegóły.</p>${historyGroup('Wygrane',wins,compactBet)}${historyGroup('Przegrane',losses,compactBet)}${historyGroup('Otwarte i pozostałe',otherBets,compactBet)}</div><div class="ineed-section"><h2>Odrzucone / wygasłe</h2><p class="ineed-muted">Ostatnie ${rejected.length} pobranych sygnałów. Starsze oceny pozostają dostępne po rozwinięciu.</p>${historyGroup('Dzisiaj',rejectedToday,compactSignal)}${historyGroup('Wcześniejsze',rejectedOlder,compactSignal)}</div>${admin()?`<div class="ineed-section"><h2>Diagnostyka admina</h2><div class="ineed-card ineed-health"><span class="ineed-pill">${esc(d.health?.status||'—')}</span><span>ostatni sync: ${dt(d.health?.last_sync_at)}</span><span>Kelly ${cfg.fractional_kelly??'—'}</span><span>EV min ${pct(cfg.minimum_net_ev)}</span><span>edge min ${cfg.minimum_edge_pp??'—'} pp</span><span>single cap ${pct(cfg.max_single_bet_pct)}</span><span>total cap ${pct(cfg.max_total_exposure_pct)}</span><span>exposure ${esc(d.risk?.source||'—')} · V1 ${n(d.risk?.v1_open_units)} · BB ${n(d.risk?.builder_open_units)}</span></div></div>`:''}</div>`;
